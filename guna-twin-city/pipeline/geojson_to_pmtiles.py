"""
GeoJSON → PMTiles Converter (Guna)
===================================
...
Usage:
    ...
    python geojson_to_pmtiles.py --input ../data/vectors/google_open_buildings_guna.geojson
...
def convert(
    ...
    # Drop rows with null/empty geometry
    before = len(gdf)
    ...
    with open(output_path, "wb") as f:
        ...
        metadata = {
            "name": f"Google Open Buildings — {CITY_NAME.title()}",
            "description": f"Building footprints from Google Open Buildings v3 for {CITY_NAME.title()}",
            "format": "pbf",
            "type": "overlay",
            "version": "1.0",
...
def main():
    default_input = Path(__file__).parent.parent / "data" / "vectors" / f"google_open_buildings_{CITY_NAME}.geojson"
