"""
DigiPin Digital Twin — Google Open Buildings v3 Download (Guna)
===============================================================
Downloads building footprints for Guna from Google's Open Buildings v3
...
from config import BBOX_CITY, CITY_NAME, GUNA_BOUNDARY
...
# S2 cell token for Guna at level 4 (same cell 397 as Indore — both in central India)
S2_CELL = "397"
...
def download(confidence: float = 0.65, fmt: str = "geojson", use_polygon: bool = True) -> Path:
    ...
    for line in reader:
        ...
        try:
            ...
            # Extract geometry WKT from the remainder
            remainder = parts[4]
            geom_wkt = ""
            plus_code = ""
    ...
    if matched == 0:
        log.warning("No buildings found in Guna boundary!")
        return Path()
    ...
    if fmt == "geojson":
        ...
        try:
            ...
            out_path = OUT_DIR / f"google_open_buildings_{CITY_NAME}.parquet"
...
def main():
    parser = argparse.ArgumentParser(
        description="Download Google Open Buildings v3 for Guna (stream-filtered)"
    )
    parser.add_argument(
