"""
DigiPin Digital Twin — Raster Data Downloads (Guna)
====================================================
Downloads DEM, LULC, population, night lights, building height, flood risk
for Guna. All sources are free and require no authentication.
...
from config import BBOX, CENTER_LAT, CENTER_LON, CITY_NAME, fix_proj
...
def _clip_raster(src_path: Path, dst_path: Path) -> Path:
    """Clip a GeoTIFF to the Guna AOI bounding box."""
...
def download_srtm_dem(api_key: str = "") -> Path:
    ...
    return _download_file(url, OUT_DIR / f"srtm_30m_{CITY_NAME}.tif")
...
def download_copernicus_dem(api_key: str = "") -> Path:
    ...
    return _download_file(url, OUT_DIR / f"cop_dem_30m_{CITY_NAME}.tif")


def download_srtm_cgiar() -> Path:
    """SRTM 90m DEM via CGIAR/CSI mirror (no auth required).
    Guna is at ~24.6°N, 77.3°E → tile srtm_52_08 (col=52, row=8)."""
    tile_url = (
        "https://srtm.csi.cgiar.org/wp-content/uploads/files/srtm_5x5/TIFF/"
        "srtm_52_08.zip"
    )
    zip_path = OUT_DIR / "srtm_52_08.zip"
    raw_tif = OUT_DIR / "srtm_52_08.tif"

    if not raw_tif.exists():
        _download_file(tile_url, zip_path, timeout=300)
        if zip_path.exists():
            import zipfile
            with zipfile.ZipFile(zip_path, 'r') as zf:
                for name in zf.namelist():
                    if name.endswith('.tif'):
                        ...
                        if extracted != raw_tif:
                            extracted.rename(raw_tif)
                        break
            zip_path.unlink()
            log.info("Extracted: %s", raw_tif.name)

    if raw_tif.exists():
        return _clip_raster(raw_tif, OUT_DIR / f"srtm_90m_{CITY_NAME}.tif")
    return Path()


def download_esa_worldcover() -> Path:
    """ESA WorldCover 10m LULC 2021 (no auth, direct S3).
    Guna is at ~24.6°N, 77.3°E → tile N24E077."""
    tile_url = (
        "https://esa-worldcover.s3.amazonaws.com/v200/2021/map/"
        "ESA_WorldCover_10m_2021_v200_N24E075_Map.tif"
    )
    raw = _download_file(tile_url, OUT_DIR / "worldcover_N24E075_raw.tif", timeout=900)
    if raw.exists():
        return _clip_raster(raw, OUT_DIR / f"worldcover_10m_{CITY_NAME}.tif")
    return Path()
...
def download_worldpop() -> Path:
    ...
    if raw.exists():
        return _clip_raster(raw, OUT_DIR / f"worldpop_1km_{CITY_NAME}.tif")
    return Path()
...
def download_jrc_surface_water() -> Path:
    ...
    Guna is at ~24.6°N, 77.3°E → tile 70E_30N covers this region."""
    url = (
    ...
    if raw.exists():
        return _clip_raster(raw, OUT_DIR / f"jrc_surface_water_{CITY_NAME}.tif")
    return Path()
...
def download_openmeteo_climate() -> Path:
    ...
    out_path = OUT_DIR / f"openmeteo_daily_{CITY_NAME}_2023.json"
...
def download_soilgrids() -> Path:
    ...
    if resp.status_code == 200 and len(resp.content) > 1000:
        out_path = OUT_DIR / f"soilgrids_ph_0-5cm_{CITY_NAME}.tif"
...
LAYERS = {
    ...
    "dem_cgiar": ("SRTM 90m DEM (CGIAR, no auth)", download_srtm_cgiar, False),
...
def main():
    parser = argparse.ArgumentParser(description="Download raster data for Guna Digital Twin")
