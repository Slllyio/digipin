"""
DigiPin Digital Twin — Real-Time Sensor Data Collection (Guna)
==============================================================
Fetches current AQI, weather, traffic, and flood data for Guna
...
from config import CENTER_LAT, CENTER_LON, CITY_NAME
...
def fetch_waqi(token: str = "") -> SensorReading:
    ...
    # Guna may not have a dedicated WAQI station — use geo-based feed
    resp = requests.get(
        f"https://api.waqi.info/feed/geo:{CENTER_LAT};{CENTER_LON}/",
...
def main():
    parser = argparse.ArgumentParser(description=f"Fetch real-time sensor data for {CITY_NAME.title()}")
