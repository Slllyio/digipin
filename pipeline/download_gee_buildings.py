"""
DigiPin Digital Twin — Google Open Buildings v3 via Earth Engine
===============================================================
Downloads ~100K+ building footprints for Indore from Google's
Open Buildings dataset using the Earth Engine Python API.

Dataset: ee.FeatureCollection("GOOGLE/Research/open-buildings/v3/polygons")
- 1.8B+ building footprints globally (V3, July 2023)
- Derived from high-resolution satellite imagery using ML
- Fields: geometry (polygon), confidence (0-1), area_in_meters, full_plus_code
...
def download_direct(confidence: float = 0.65, fmt: str = "geojson") -> Path:
    ...
    log.info("Querying Google Open Buildings v3 for Indore (confidence >= %.2f)...", confidence)
...
def export_to_drive(confidence: float = 0.65) -> str:
    ...
    task = ee.batch.Export.table.toDrive(
        ...
        description="google_open_buildings_indore",
        folder="digipin_data",
        fileNamePrefix="google_open_buildings_indore",
        fileFormat="GeoJSON",
    )
    task.start()
    task_id = task.id
...
def print_dataset_info():
    ...
    print(f"\n{'='*50}")
    print(f"Google Open Buildings v3 — Indore")
    print(f"{'='*50}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Download Google Open Buildings v3 for Indore via Earth Engine"
    )
    parser.add_argument(
